...
<script lang="ts">
  ...
  interface BackendBridgeStatus {
    running: boolean;
    connected: boolean;
    mappingCount: number;
    lastError?: string | null;
    broker?: { host?: string } | null;
  }

  interface BackendBridgeLogEvent {
  ...
  function applyBackendStatus(status: BackendBridgeStatus): void {
    setBridgeStatus({
      running: status.running,
      connected: status.connected,
      mappingCount: status.mappingCount,
      lastError: status.lastError ?? null,
      brokerHost: status.broker?.host ?? null,
    });
  }
  ...
  onMount(() => {
    ...
    void (async () => {
      try {
        const status = await invoke<BackendBridgeStatus>("get_cloud_bridge_status");
        applyBackendStatus(status);
      } catch (err) {
        addLog("warn", `Cloud bridge status fetch failed: ${err}`);
      }

      try {
        const off = await listen<BackendBridgeStatus>("cloud-bridge://status", (event) => {
          applyBackendStatus(event.payload);
        });
        unlisteners.push(off);
      } catch (err) {
        addLog("warn", `Failed to subscribe to cloud-bridge://status: ${err}`);
      }

      try {
        const off = await listen<BackendBridgeLogEvent>("cloud-bridge://event", (event) => {
          const payload = event.payload;
          appendBridgeLog({
            level: payload.level,
            message: `[${payload.topic}] ${payload.message}`,
          });
          // Mirror into the global app log so the user has a single source of truth.
          addLog(payload.level, `[CLOUD-BRIDGE/${payload.topic}] ${payload.message}`);
        });
        unlisteners.push(off);
      } catch (err) {
        addLog("warn", `Failed to subscribe to cloud-bridge://event: ${err}`);
      }
    })();
  ...
  async function handleStart(): Promise<void> {
    ...
    try {
      ...
      const status = await invoke<BackendBridgeStatus>("start_cloud_bridge", { request });
      applyBackendStatus(status);
  ...
  async function handleStop(): Promise<void> {
    ...
    busy = true;
    try {
      const status = await invoke<BackendBridgeStatus>("stop_cloud_bridge");
      applyBackendStatus(status);
