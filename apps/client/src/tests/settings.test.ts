// Settings state — persistence, normalization, formatters
...
describe("getGlobalPollingMaxAddressCount / isPollingAllowedForCount", () => {
  it("getGlobalPollingMaxAddressCount returns current max", () => {
    setGlobalPollingMaxAddressCount(50);
    expect(getGlobalPollingMaxAddressCount()).toBe(50);
  });

  it("isPollingAllowedForCount returns true when count ≤ max", () => {
    setGlobalPollingMaxAddressCount(100);
    expect(isPollingAllowedForCount(100)).toBe(true);
    expect(isPollingAllowedForCount(99)).toBe(true);
  });

  it("isPollingAllowedForCount returns false when count > max", () => {
    setGlobalPollingMaxAddressCount(100);
...
describe("setTcpHeartbeatEnabled", () => {
  it("sets heartbeat enabled to false", () => {
...
describe("setFeatureDefaults", () => {
  ...
  it("updates view to 'switch' for coils", () => {
    setFeatureDefaults("coils", { view: "switch" });
    expect(settingsState.defaults.coils.view).toBe("switch");
  });

  it("does not affect other feature defaults when updating one", () => {
...
describe("getSettingsSnapshot", () => {
  it("returns a deep copy that does not mutate state when modified", () => {
    const snap = getSettingsSnapshot();
    (snap as Record<string, unknown>).valueViewFormat = "hex";
    expect(settingsState.valueViewFormat).toBe("dec"); // unchanged
  });
...
describe("formatAddressWithSettings", () => {
  it("formats as zero-padded decimal in 'dec' mode", () => {
    setValueViewFormat("dec");
    expect(formatAddressWithSettings(42)).toBe("0042");
  });

  it("formats as 0x-prefixed hex in 'hex' mode", () => {
    setValueViewFormat("hex");
  ...
  it("clamps negative values to 0", () => {
    setValueViewFormat("dec");
    expect(formatAddressWithSettings(-5)).toBe("0000");
  });

  it("floors fractional addresses", () => {
    setValueViewFormat("dec");
    expect(formatAddressWithSettings(10.9)).toBe("0010");
  });
});
...
describe("formatWordValueWithSettings", () => {
  it("formats as plain integer in 'dec' mode", () => {
    setValueViewFormat("dec");
  ...
  it("clamps to 65535 for over-range values", () => {
    setValueViewFormat("dec");
...
describe("enforceLogRetention", () => {
  it("returns the original array if entries ≤ maxRetainedEntries", () => {
  ...
  it("trims to the last maxRetainedEntries entries when over the limit", () => {
    setMaxRetainedLogEntries(4000);
    const entries = Array.from({ length: 4100 }, (_, i) => ({ id: i }));
    const result = enforceLogRetention(entries);
    expect(result).toHaveLength(4000);
    expect(result[0].id).toBe(100); // first 100 trimmed
  ...
  it("never returns fewer than 200 entries (floor)", () => {
    setMaxRetainedLogEntries(50); // below minimum; getSettings normalizes to 200
