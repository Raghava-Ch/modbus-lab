...
<script lang="ts">
  ...
  import {
    ...
    setValueViewFormat,
    settingsState,
    exportSettingsAndConnection,
    importSettingsAndConnectionJson,
  } from "../../state/settings.svelte";

  let configFileInput = $state<HTMLInputElement>();
  ...
  function handleExportConfig(): void {
    try {
      const content = exportSettingsAndConnection();
      const blob = new Blob([content], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `modbus-lab-config-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
      a.click();
  ...
  function handleImportConfig(event: Event): void {
    ...
    if (target.files && target.files[0]) {
      ...
      reader.onload = (e) => {
        ...
        importSettingsAndConnectionJson(text);
        target.value = "";
      };
      ...
      <div class="settings-grid">
        ...
        <section class="group">
          ...
          <div class="backup-restore-row">
            <button class="ctrl-btn" type="button" onclick={handleExportConfig}>
              Export Configuration
            </button>
            <button class="ctrl-btn" type="button" onclick={() => configFileInput?.click()}>
          ...
          <p class="note">
            Save or load your settings and connection settings to a local JSON file.
          </p>
        </section>
      </div>
    {/snippet}
