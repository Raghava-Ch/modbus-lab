...
<script lang="ts">
  ...
  import { notifyWarning, notifyInfo } from "../../state/notifications.svelte";
  import {
    addFifoAddress,
    fifoState,
    initFifoState,
  ...
  let fifoFileInput = $state<HTMLInputElement>();
  ...
  function handleExportFifo(): void {
    try {
      const content = exportFifoState();
      const blob = new Blob([content], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `modbus-lab-fifo-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error("Failed to export FIFO queues:", error);
    }
  }

  function handleImportFifo(event: Event): void {
      ...
      <PanelFrame>
        {#snippet children()}
          <div class="manage-panel">
            ...
            <div class="fifo-backup-row">
              <button class="ctrl-btn" type="button" onclick={handleExportFifo}>
                Export Queues
              </button>
              <button class="ctrl-btn" type="button" onclick={() => fifoFileInput?.click()}>
                Import Queues
              </button>
              <input
                type="file"
                accept=".json"
                bind:this={fifoFileInput}
                onchange={handleImportFifo}
                style="display: none;"
              />
            </div>
          </div>
