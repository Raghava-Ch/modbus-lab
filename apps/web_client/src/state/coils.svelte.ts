...
import type { BackendReadCoilsResponse, BackendWriteCoilResponse, BackendWriteMassCoilsResponse } from "../lib/adapters/IModbusAdapter";
...
export async function writeCoil(address: number): Promise<void> {
  ...
  entry.writeError = null;
  try {
    const response = await modbusAdapter.writeCoil(address, valueToWrite);
...
export async function readCoil(address: number): Promise<void> {
  ...
  entry.pending = true;
  try {
    const response = await modbusAdapter.readCoils(address, 1);
...
async function writeAddressMap(
  ...
  try {
    const response = await modbusAdapter.writeCoilsBatch(coils);
  ...
  for (const section of sections) {
    try {
      if (section.quantity === 1) {
        const response = await modbusAdapter.readCoils(section.start, 1);
      ...
      for (let chunkStart = section.start; chunkStart < section.start + section.quantity; chunkStart += COIL_MAX_COUNT) {
        ...
        const response = await modbusAdapter.readCoils(chunkStart, chunkQty);
