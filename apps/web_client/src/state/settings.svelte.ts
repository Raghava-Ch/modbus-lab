...
import { notifyInfo, notifyError } from "./notifications.svelte";
...
function normalizeSettings(raw: unknown): AppSettings {
  ...
  base.ibus.autoProbeOnConnect =
    typeof ibus.autoProbeOnConnect === "boolean" ? ibus.autoProbeOnConnect : base.ibus.autoProbeOnConnect;
...
export function setIbusAutoProbeOnConnect(value: boolean): void {
  settingsState.ibus.autoProbeOnConnect = value;
  persist();
}
...
export function exportSettingsAndConnection(): string {
  const payload = {
    schema: "modbus-lab.config.v1",
    ...
    connection: {
      ...
      tcp: {
        host: connectionState.tcp.host,
        port: connectionState.tcp.port,
        wsProxyUrl: connectionState.tcp.wsProxyUrl,
      ...
      serial: {
        port: connectionState.serial.port,
...
export function importSettingsAndConnectionJson(raw: string): void {
  try {
    const parsed = JSON.parse(raw);
    if (!parsed || parsed.schema !== "modbus-lab.config.v1") {
      throw new Error("Invalid or unsupported configuration schema version.");
    }
    ...
    if (parsed.connection) {
      ...
      if (conn.protocol) connectionState.protocol = conn.protocol;
      if (typeof conn.slaveId === "number") connectionState.slaveId = conn.slaveId;
      if (conn.tcp) {
        connectionState.tcp = {
          ...connectionState.tcp,
          ...conn.tcp,
    ...
    notifyInfo("Configuration imported successfully.");
  } catch (err) {
    const message = err instanceof Error ? err.message : "Invalid config JSON.";
    notifyError(`Config import failed: ${message}`);
  }
}
