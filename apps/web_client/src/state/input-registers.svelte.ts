...
import type { BackendReadInputRegistersResponse } from "../lib/adapters/IModbusAdapter";
...
export async function readInputRegister(address: number): Promise<void> {
  ...
  try {
    const response = await modbusAdapter.readInputRegisters(address, 1);
...
async function readInputRange(
  ...
  return modbusAdapter.readInputRegisters(startAddress, quantity);
}
