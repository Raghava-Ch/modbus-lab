...
const FILE_RECORD_SCENARIOS_KEY = "Modbus-Lab.server.file-records.scenarios";
...
export async function executeFileRecord(notifyOnSuccess: boolean = true): Promise<void> {
	...
	try {
		...
		addLog(
			"info",
			`[FILE-RECORDS] store.${formatFcLabel(functionCode)} ok ${summarizeSegments(mode, requestSegments)} rsp=${response.responseHex}`,
		);
