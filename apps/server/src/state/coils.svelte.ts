...
export type CoilRuleType = "none" | "auto-toggle";

export interface CoilRule {
  type: CoilRuleType;
  intervalMs: number;
}

export const DEFAULT_RULE: CoilRule = { type: "none", intervalMs: 1000 };
...
export const coilState = $state({
  ...
  addressFilter: "all" as CoilAddressFilter,
  addressRangeStart: 0,
  addressRangeEnd: 0,
...
export function setCoilAddressFilter(filter: CoilAddressFilter): void {
  coilState.addressFilter = filter;
}

export function setCoilAddressRange(startAddress: number, endAddress: number): void {
  ...
  coilState.addressRangeStart = Math.min(start, end);
  coilState.addressRangeEnd = Math.max(start, end);
}

export function setCoilAddressList(addresses: number[]): void {
  const normalized = addresses
    .map((a) => Math.floor(a))
    .filter((a) => Number.isFinite(a) && a >= MODBUS_ADDRESS_MIN && a <= MODBUS_ADDRESS_MAX);
  coilState.addressList = [...new Set(normalized)].sort((a, b) => a - b);
}
...
export async function writeCoil(address: number): Promise<void> {
  ...
  entry.writeError = null;
  try {
    const response = await invoke<BackendWriteCoilResponse>("store_write_coil", {
...
async function writeAddressMap(
  ...
  try {
    const response = await invoke<BackendWriteMassCoilsResponse>("store_write_coils_batch", {
      request: { coils },
    });
...
export function addExclusiveCoil(address: number): boolean {
  ...
  void invoke("store_write_coil", { request: { address: normalized, value: false } });
  return true;
}
...
export function getCoilRule(address: number): CoilRule {
  return coilRules[address] ?? DEFAULT_RULE;
}

export function setCoilRule(address: number, rule: CoilRule): void {
  stopRuleTimer(address);
  if (rule.type === "none") {
    delete coilRules[address];
  } else {
    coilRules[address] = { ...rule };
    if (rule.type === "auto-toggle") {
      const timer = setInterval(() => {
        ...
        toggleCoilValue(address);
        void writeCoil(address);
      }, rule.intervalMs);
...
export function clearAllRules(): void {
  ...
  ruleTimers.clear();
  for (const key of Object.keys(coilRules)) {
    delete coilRules[Number(key)];
  }
}
...
export function syncCoilAddressesToBackend(): void {
  const addresses = coilState.entries.map((e) => e.address);
  void invoke("store_sync_coil_addresses", { addresses });
}
...
export function getFilteredCoils(): CoilEntry[] {
  const valueFiltered = (() => {
    switch (coilState.filter) {
      case "on":
        return coilState.entries.filter((entry) => entry.slaveValue);
      case "off":
        return coilState.entries.filter((entry) => !entry.slaveValue);
      default:
  ...
  const rangeStart = Math.min(coilState.addressRangeStart, coilState.addressRangeEnd);
  const rangeEnd = Math.max(coilState.addressRangeStart, coilState.addressRangeEnd);
  const inRange = (address: number): boolean => address >= rangeStart && address <= rangeEnd;
  const listSet = new Set(coilState.addressList);

  switch (coilState.addressFilter) {
    case "required-range":
