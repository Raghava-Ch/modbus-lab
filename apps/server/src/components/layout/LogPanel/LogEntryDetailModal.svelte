...
<script lang="ts">
  ...
  function groupModbusTcpBytes(bytesStr: string): ByteGroup[] {
    ...
    const groups: ByteGroup[] = [
      { label: "Transaction ID", bytes: bytes.slice(0, 2), color: "var(--c-accent)" },
      { label: "Protocol ID",    bytes: bytes.slice(2, 4), color: "var(--c-text-2)" },
      { label: "Length",         bytes: bytes.slice(4, 6), color: "var(--c-text-2)" },
      { label: "Unit ID",        bytes: bytes.slice(6, 7), color: "var(--c-warn)" },
      { label: "Function Code",  bytes: bytes.slice(7, 8), color: "var(--c-accent)" },
    ];
    if (bytes.length > 8) {
...
<style>
  ...
  .byte-group {
    ...
    min-width: calc(var(--group-label-ch, 8) * 1ch + 20px);
    display: flex;
    flex-wrap: wrap;
    gap: 4px;
