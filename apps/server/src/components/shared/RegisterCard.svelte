...
<script lang="ts">
  ...
  import type { HoldingRegRule } from "../../state/holding-registers.svelte";
  import { RULE_INTERVAL_OPTIONS } from "../../state/holding-registers.svelte";
  ...
  let {
    ...
    onRuleChange: (address: number, nextRule: HoldingRegRule) => void;
  ...
  let editRuleType = $state<HoldingRegRule["type"]>("none");
  ...
  function isWaveRule(type: HoldingRegRule["type"]): boolean {
    return type !== "none";
  }

  function ruleSummary(ruleValue: HoldingRegRule): string {
      ...
      >
        <div class="rule-pop-grid">
          <label class="rule-pop-field">
            ...
            <select value={editRuleType} onchange={(e) => { editRuleType = e.currentTarget.value as HoldingRegRule["type"]; }}>
